...
import helpers.ks_imageloader_custom as kl
...
handler = logging.FileHandler(f"multi_view_{current_time}.log")
...
def main(config):
    ...
    # multi view
        ...
        img_size = 227
    elif config['model_to_use'] == "alexnet_mv_max_color":
        logging.info("using multiview alexnet max model for color proposal")
        from models.alexnet_mv_color import AlexnetMax
        model = AlexnetMax(hparams={"lr": 0.0002}, num_classes=config['num_classes'],
                           pretrained=True, seed=config['manualSeed'])
        checkpoint = torch.load(config['backbone_path'], map_location=lambda storage, loc: storage)
        keys = OrderedDict({k: v for k, v in checkpoint['state_dict'].items() if 'classifier' not in k})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml', default='config_multiview.yaml')
